...
class PartDrag4DEvalDatset(Dataset):
    ...
    def process_state_data(self, uid, zero123_uid, vids, is_input=False):
        images = []
        masks = []
        cam_poses = []
        camera_matrices = []
        ...
        for vid in vids:
            try:
                ...
                if is_input and vid_cnt > 0 and vid_cnt < 4:
                    image_path = os.path.join(zero123_uid, f'{vid:03d}_rgba.png')
    ...
    def __getitem__(self, idx):
        ...
        zero123_uid = self.zero123_items[idx]
        ...
        # Randomly extract 1 drag from the moving part's surface
        ...
        if os.path.exists(os.path.join(self.opt.mesh_base, cur_class, parts_out[0], 'motion', f"{statei_id}_cam0_visible.npy")):
            ...
            rand_idx = np.random.permutation(inter_idx)[:1]
        else:
            rand_idx = np.array([])
            
        # Get drag
        if rand_idx.shape[0] > 0:
            ...
            rand_idx = np.random.permutation(start_action.shape[0])[:1]
        ...
        images, masks, cam_poses, camera_matrix = self.process_state_data(uid, zero123_uid, vids, is_input=True)
        images_output, masks_output, cam_poses_output, camera_matrix_output = self.process_state_data(uid_output, zero123_uid, vids, is_input=False)
        ...
        results['drags_start'] = results['drags_start'].repeat(1, self.opt.num_drags, 1)
        ...
        for v in range(results['drags_start'].shape[0]):
            for n in range(results['drags_start'].shape[1]):
                ...
                results['drags_end'][v, n] = results['drags_start'][v, n] + (results['drags_end'][0, 0] - results['drags_start'][0, 0])
        results['drags_end'] = torch.clamp(results['drags_end'], 0, 511)
