...
from core.drag_embedding import DragPositionNet, FourierEmbedder, DragPositionNetMultiScale
...
class UNet(nn.Module):
    def __init__(
        ...
        self.use_drag_encoding = use_drag_encoding
        
        if use_drag_encoding:
            self.drag_position_net = DragPositionNet(num_drags=10, fourier_freqs=8)
            # self.alpha_drag = nn.Parameter(torch.tensor(0.0), requires_grad=True)
            self.conv_merge = nn.Conv2d(in_channels=2048, out_channels=1024, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, drags_start=None, drags_end=None):
        ...
        if self.use_drag_encoding:
            drags_embeddings = self.drag_position_net(drags_start, drags_end)
            x = self.conv_merge(torch.cat([x, drags_embeddings], dim=1))
            # x += drags_embeddings * self.alpha_drag
        x = self.mid_block(x)
...
class UNetWithMSDrag(nn.Module):
    def __init__(
        self,
        in_channels: int = 3,
        out_channels: int = 3,
        ...
        skip_scale: float = np.sqrt(0.5),
        use_drag_encoding = False,
        drag_scales = [256, 128, 64, 32, 16, 8],
        drag_channels = [64, 64, 128, 256, 512, 1024],
        scale_layer_idx=None
    ):
        super().__init__()
        ...
        if use_drag_encoding:
            self.drag_position_net = DragPositionNetMultiScale(fourier_freqs=8, scales=drag_scales, channels=drag_channels, drag_layer_idx=scale_layer_idx)
            self.conv_merges = []
            self.scale_layer_idx = scale_layer_idx
            for i in range(len(drag_scales)):
                if scale_layer_idx is not None and i != scale_layer_idx:
                    continue
    ...
    def forward(self, x, drags_start=None, drags_end=None):
        ...
        # Get Drag Embeddings
        if self.use_drag_encoding:
            drags_start_embeddings_ms, drags_end_embeddings_ms = self.drag_position_net(drags_start, drags_end)
