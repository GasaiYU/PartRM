...
class ObjaverseHQTrainDataset(Dataset):
    def __init__(self, opt: Options):
        ...
        filelist = self.opt.train_filelist
        ...
        for action_dir in action_dirs:
            if os.path.isdir(action_dir):
                ...
                if not self.judge_action(action_dir):
                    continue
                for cur_frame_id, image in enumerate(valid_image_list):
                    ...
                    for deform_frame_id, other_action_image in enumerate(other_action_images):
                        ...
                        deform_objaverse_id = deform_image_path.split('/')[-3]
                        deform_objavser_drag_root = deform_image_path.split('/')[-2]
                        deform_objaverse_image_name = deform_image_path.split('/')[-1].split('.')[0]
                        deform_gs_name = '_'.join([deform_objaverse_id, deform_objavser_drag_root, deform_objaverse_image_name])
                        self.deform_gt_gaussian_paths.append(os.path.join(opt.base_file_path, f"eval_gaussians_{deform_gs_name}.ply"))
    ...
    def process_state_data(self, image_path0, camera_dir, vids):
        images = []
        masks = []
        cam_poses = []
        camera_matrixes = []
    ...
    def __getitem__(self, idx):
        ...
        images, masks, cam_poses, camera_matrixes = self.process_state_data(image_path0, camera_dir, vids)
        images_output, masks_output, cam_poses_output, camera_matrix_output = self.process_state_data(image_path0_output, camera_dir_output, vids)
        #######################################################
        ...
        results['gt_gaussians'] = self.load_ply(self.deform_gt_gaussian_paths[idx])
